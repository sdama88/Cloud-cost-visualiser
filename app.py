...
selected_workload = st.selectbox("Select a workload type", workload_names, index=workload_names.index(default_workload))

# Slider for concurrent users
concurrent_users = st.slider("Number of Concurrent Users", min_value=1, max_value=max_users, value=100, step=1)
...
with col1:
    st.metric(label="Estimated Monthly Cloud Cost*", value=f"{currency} {total_cost:,.0f}")
with col2:
    st.metric(label="Estimated GPUs Needed", value=f"{gpu_count}")

# Chart: Cost vs Concurrency
chart_data = []
for users in range(1, max_users+1, max(1, max_users // 50)):  # 50 points for smoothness
